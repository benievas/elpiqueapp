...
export default function OwnerFeedPage() {
  ...
  async function load() {
    ...
    const { data } = await supabase
      .from("feed_posts")
      .select("id, titulo, contenido, tipo, imagen_principal, visible, fecha_expiracion, created_at")
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (editingId) {
        const { error: e } = await supabaseMut.from("feed_posts").update(payload).eq("id", editingId);
        if (e) throw e;
      } else {
        const { error: e } = await supabaseMut.from("feed_posts").insert(payload);
        if (e) throw e;
      }
      await load();
  ...
  function startEdit(p: Post) {
    setEditingId(p.id);
    const diasRestantes = p.fecha_expiracion
      ? Math.max(0, Math.ceil((new Date(p.fecha_expiracion).getTime() - Date.now()) / (24*60*60*1000)))
      : 0;
    setForm({
      titulo: p.titulo,
  ...
  if (!activeComplexId) {
    return <div className="text-rodeo-cream/50 text-sm py-12 text-center">Seleccioná un complejo para gestionar su feed.</div>;
  }
    ...
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-center gap-4">
        <div className="flex-1">
          ...
          <h1 style={{ fontFamily: "'Barlow Condensed', system-ui, sans-serif", fontWeight: 900, fontSize: "36px", letterSpacing: "-0.02em", textTransform: "uppercase", lineHeight: 0.95 }} className="text-white"><span className="section-slash">/</span>Feed</h1>
        </div>
      ...
      {loading ? (
        <div className="space-y-2">{Array.from({ length: 3 }).map((_, i) => <div key={i} style={{ height: 80, borderRadius: 14, background: "linear-gradient(90deg,rgba(255,255,255,0.04) 25%,rgba(255,255,255,0.08) 50%,rgba(255,255,255,0.04) 75%)", backgroundSize: "200% 100%", animation: "skeleton-shimmer 1.5s infinite" }} />)}</div>
      ) : posts.length === 0 ? (
        <div className="text-center py-16 space-y-3">
          <Newspaper size={32} className="mx-auto text-rodeo-cream/15" />
          <p className="text-rodeo-cream/30 text-sm">Todavía no publicaste nada</p>
        </div>
      ) : (
        <div className="space-y-2">
          {posts.map((p, i) => {
            const vencido = p.fecha_expiracion && new Date(p.fecha_expiracion) < new Date();
            return (
              <motion.div key={p.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}
                ...
                <div className="flex-1 min-w-0 py-3">
                  ...
                  <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                    ...
                    {vencido && <span className="text-[10px] font-bold text-red-400/70">VENCIDO</span>}
                    {p.fecha_expiracion && !vencido && <span className="text-[10px] text-rodeo-cream/30">vence {new Date(p.fecha_expiracion).toLocaleDateString("es-AR")}</span>}
                  </div>
                </div>
                <div className="flex items-center gap-1 shrink-0">
                  <button onClick={() => toggleVisible(p)} disabled={toggling === p.id} className="p-2 hover:bg-white/10 rounded-lg" title={p.visible ? "Ocultar" : "Publicar"}>
                  ...
                  <button onClick={() => startEdit(p)} className="p-2 hover:bg-white/10 rounded-lg">
                    <Edit2 size={13} className="text-rodeo-cream/40 hover:text-rodeo-lime" />
                  </button>
                  <button onClick={() => handleDelete(p.id)} disabled={deleting === p.id} className="p-2 hover:bg-white/10 rounded-lg">
                    {deleting === p.id ? <RefreshCw size={13} className="animate-spin text-red-400/50" /> : <Trash2 size={13} className="text-rodeo-cream/30 hover:text-red-400" />}
                  </button>
                </div>
              </motion.div>
            );
